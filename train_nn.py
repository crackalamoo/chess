...
class TrainCallback(tf.keras.callbacks.Callback):
    ...
    def on_epoch_end(self, epoch, logs=None):
        print("\nEvaluating model")
        results = self.model.evaluate([self.tX0, self.tX1], [self.ty], batch_size=BATCH_SIZE)
...
results = model.evaluate([test_X0, test_X1], test_y, batch_size=BATCH_SIZE)
print("|| test loss, test acc", results)
...
history = model.fit(
    ...
    epochs=NUM_EPOCHS,
    callbacks=[TrainCallback(model, test_X0, test_X1, test_y)]
)
